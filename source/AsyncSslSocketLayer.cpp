...
typedef int			(*tSSL_get_ex_data_X509_STORE_CTX_idx) (void);
typedef int			(*tSSL_CTX_load_verify_locations)	(SSL_CTX *ctx, const char *CAfile, const char *CApath);
...
typedef SSL*				(*tX509_STORE_CTX_get_ex_data)		(X509_STORE_CTX *ctx, int idx);
typedef char*				(*tX509_NAME_oneline)				(X509_NAME *a, char *buf, int size);
typedef char*				(*tX509_verify_cert_error_string)	(int err);
typedef int					(*tX509_STORE_CTX_get_error_depth)	(X509_STORE_CTX *ctx);
...
static tSSL_get_ex_data_X509_STORE_CTX_idx pSSL_get_ex_data_X509_STORE_CTX_idx;
static tSSL_CTX_load_verify_locations	pSSL_CTX_load_verify_locations;
static tSSL_ctrl					pSSL_ctrl;
...
static tX509_STORE_CTX_get_ex_data		pX509_STORE_CTX_get_ex_data;
static tX509_NAME_oneline				pX509_NAME_oneline;
static tX509_verify_cert_error_string	pX509_verify_cert_error_string;
static tX509_STORE_CTX_get_error_depth	pX509_STORE_CTX_get_error_depth;
...
bool CAsyncSslSocketLayer::InitSSL()
{
	...
	if (!m_nSslRefCount)
	{
		...
		pX509_STORE_CTX_get_ex_data			= (tX509_STORE_CTX_get_ex_data)			GetProcAddress(m_hSslDll2, "X509_STORE_CTX_get_ex_data");
		pX509_NAME_oneline					= (tX509_NAME_oneline)					GetProcAddress(m_hSslDll2, "X509_NAME_oneline");
		pX509_verify_cert_error_string		= (tX509_verify_cert_error_string)		GetProcAddress(m_hSslDll2, "X509_verify_cert_error_string");
		pX509_STORE_CTX_get_error_depth		= (tX509_STORE_CTX_get_error_depth)		GetProcAddress(m_hSslDll2, "X509_STORE_CTX_get_error_depth");
		...
		if (!pBIO_ctrl_pending					||
			...
			!pX509_NAME_oneline					||
			!pX509_verify_cert_error_string		||
			!pX509_STORE_CTX_get_error_depth)
		{
			FreeLibrary(m_hSslDll1);
			m_hSslDll1=0;
			FreeLibrary(m_hSslDll2);
		...
		pSSL_get_ex_data_X509_STORE_CTX_idx = (tSSL_get_ex_data_X509_STORE_CTX_idx)	GetProcAddress(m_hSslDll1, "SSL_get_ex_data_X509_STORE_CTX_idx");
		pSSL_CTX_load_verify_locations = (tSSL_CTX_load_verify_locations) GetProcAddress(m_hSslDll1, "SSL_CTX_load_verify_locations");
		pSSL_ctrl					= (tSSL_ctrl)					GetProcAddress(m_hSslDll1, "SSL_ctrl");
...
int CAsyncSslSocketLayer::verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
	...
	if (preverify_ok)
		pLayer->m_nVerificationResult = 0;
	else
	{
