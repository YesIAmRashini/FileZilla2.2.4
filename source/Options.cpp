...
void COptions::SetOption(int nOptionID,int value)
{
	...
	if (m_bUseXML)
	{
		...
		m_markup.Save( ((CFileZillaApp *)AfxGetApp())->m_appPath + _T("FileZilla.xml") );
	}
	else
	{
		unsigned char tmp[4];
...
void COptions::Init()
{
	...
	CFileStatus status;
	if (!CFile::GetStatus( ((CFileZillaApp *)AfxGetApp())->m_appPath + _T("FileZilla.xml"), status) )
		return;
	...
	if (m_markup.Load( ((CFileZillaApp *)AfxGetApp())->m_appPath + _T("FileZilla.xml")) )
	{
...
BOOL COptions::CheckUseXML()
{
	...
	{
		CFileStatus status;
		BOOL res = CFile::GetStatus( ((CFileZillaApp *)AfxGetApp())->m_appPath + _T("FileZilla.xml"), status);
